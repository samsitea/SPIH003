            ...
            <div className="mt-16 grid lg:grid-cols-6 gap-5">
                {SERVICES.map((s, i) => (
                    <Reveal key={s.title} delay={i * 0.07} className={s.span}>
                        <div
                            className="group relative h-full bg-slate-900/50 border border-white/10 rounded-2xl p-7 overflow-hidden hover:-translate-y-1.5 hover:border-neon/50 hover:shadow-[0_0_24px_rgba(232,121,249,0.14)] transition-[transform,border-color,box-shadow] duration-300"
